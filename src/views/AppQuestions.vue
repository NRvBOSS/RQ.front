<template>
  <div class="min-h-screen bg-gray-50 pb-20">
    ...
    <div class="sticky top-0 bg-white shadow-md z-50">
      <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        <div class="text-xl font-bold text-blue-600">
          Question {{ currentPage * 10 - 9 }}-{{ Math.min(currentPage * 10, totalQuestions) }} of {{ totalQuestions }}
        </div>
        <div class="bg-blue-100 px-4 py-2 rounded-full text-blue-800 font-bold shadow-inner">
    ...
    <div class="container mx-auto px-4 mt-6">
      <div v-for="(q, index) in visibleQuestions" :key="q.id" 
           class="mb-8 p-6 rounded-xl shadow-sm border border-gray-200 bg-white transition-all hover:shadow-md"
           :class="{
             'border-green-200 bg-green-50': submitted && answers[q.id] === q.correct,
             'border-red-200 bg-red-50': submitted && answers[q.id] && answers[q.id] !== q.correct
           }">
        <p class="font-bold text-lg text-gray-800 mb-4">
          <span class="text-blue-600">{{ (currentPage - 1) * 10 + index + 1 }}.</span> {{ q.text }}
        </p>
        <div class="grid gap-3">
          <label v-for="option in q.options" 
                 ...
                 :class="{
                   ...
                   'bg-red-100': submitted && answers[q.id] === option.value && option.value !== q.correct
                 }">
            <input
              ...
              :disabled="submitted"
            />
            <span class="font-medium">{{ option.key }}. {{ option.value }}</span>
            <div class="ml-auto">
              <span
              ...
              <span
                ...
                v-if="submitted && answers[q.id] === option.value && option.value !== q.correct"
              >
                ✗
              </span>
            </div>
      ...
      <div v-if="currentPage * 10 >= totalQuestions" class="flex justify-center mt-10">
        <button
          @click="checkAnswers"
      ...
      <!-- Result -->
      <div v-if="submitted" class="mt-12 p-6 bg-white rounded-xl shadow-md border border-gray-200">
        ...
        <div class="flex justify-center gap-12">
          ...
          <div class="text-center">
            <p class="text-4xl font-bold text-blue-500">{{ Math.round((t / totalQuestions) * 100) }}%</p>
